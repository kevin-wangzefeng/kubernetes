...
// ensures the PodAntiAffinity is denied if it defines topologykey anything other than kubernetes.io/hostname.
func TestInterPodAffinityAdmission(t *testing.T) {
	...
	tests := []struct {
		...
		// what ever topologyKey in preferredDuringSchedulingIgnoredDuringExecution its success.
		{
			affinity: map[string]string{
		...
		// wrong topologyKey in requiredDuringSchedulingIgnoredDuringExecution then admission fails.
		{
			affinity: map[string]string{
				api.AffinityAnnotationKey: `
					{"podAntiAffinity": {
		...
		// wrong topologyKey in requiredDuringSchedulingRequiredDuringExecution then admission fails.
