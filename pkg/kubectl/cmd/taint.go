...
A taint must begin with a letter or number, and may contain letters, numbers, hyphens, dots, and underscores, up to %[1]d characters.`
	taint_example = `# Update node 'foo' with the taint 'dedicated', the value 'banana' and the taint effect 'NoSchedule'.
...
func validateNoTaintOverwrites(node *api.Node, taints []api.Taint) error {
	...
	for _, taint := range taints {
		for _, existingTaint := range node.Spec.Taints {
			if reflect.DeepEqual(taint, existingTaint) {
...
func parseTaints(spec []string) ([]api.Taint, error) {
	...
	for _, taintSpec := range spec {
		if strings.Index(taintSpec, "=") != -1 && strings.Index(taintSpec, ":") != -1 {
			...
			if effect != api.TaintEffectNoSchedule && effect != api.TaintEffectPreferNoSchedule {
				return nil, fmt.Errorf("invalid taint spec: %v, unsupported taint effect", taintSpec)
			}
...
func RunTaint(f *cmdutil.Factory, out io.Writer, cmd *cobra.Command, args []string) error {
	...
	taints, err := parseTaints(taintArgs)
	if err != nil {
	...
	message := "tainted"
	if !reflect.DeepEqual(targetNode.Spec.Taints, newNode.Spec.Taints) {
