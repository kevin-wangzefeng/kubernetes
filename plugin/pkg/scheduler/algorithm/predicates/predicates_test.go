...
func TestPodFitsSelector(t *testing.T) {
	tests := []struct {
						...
						SoftNodeAffinity: []api.SoftNodeAffinityTerm{
							{
								Weight:           2,
								MatchExpressions: []api.NodeSelectorRequirement{
									{Key: "key2", Operator: api.NodeSelectorOpNotIn, Values: []string{"value1", "value2"}},
								},
							},
						},
					},
		...
		},
		{
			...
			fits: false,
			test: "node labels don't match pod hard node Affinity requirements",
		},
		{
			pod: &api.Pod{
				Spec: api.PodSpec{
		...
		},
		{
			...
			fits: false,
			test: "same labels with Affinity with empty NodeSelectTerms",
		},
		{
			pod: &api.Pod{
				Spec: api.PodSpec{
