...
var map_Toleration = map[string]string{
	...
	"key":                "Key is the taint key that the toleration applies to. Empty means match all taint keys. If the key is empty, operator must be Exists, which means to match all values and all keys.",
	...
	"forgivenessSeconds": "ForgivenessSeconds represents the period of time the toleration (which must be of effect NoExecute, otherwise this field is ignored) tolerates the taint. By default, it is not set, which means tolerate the taint forever (do not evict). Zero and negative values are not allowed.",
}
