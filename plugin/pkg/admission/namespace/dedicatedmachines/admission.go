...
func init() {
	admission.RegisterPlugin("NamespaceDedicatedMachines", func(client client.Interface, config io.Reader) (admission.Interface, error) {
...
func (d *dedicated) Admit(a admission.Attributes) (err error) {
	...
	_, exists, err := d.store.Get(namespace)
	if err != nil {
		return errors.NewInternalError(err)
	}
	if exists {
	...
	_, err = d.client.Namespaces().Get(a.GetNamespace())
	if err != nil {
