...
// PodToleratesNodeTaints matches pod's tolerations against node's taints if the effect is one of the following
// 1. NoSchedule
// 2. NoScheduleNoAdmit
// 3. NoScheduleNoAdmitNoExecute
// If the effect is PreferNoSchedule, then it's skipped.
// If all of a node's taints are tolerated by at least one of the pod's tolerations,this node is a candidate for scheduling
// the pod.
func (t *TolerationMatch) PodToleratesNodeTaints(pod *api.Pod, existingPods []*api.Pod, nodeID string) (bool, error) {
	node, err := t.info.GetNodeInfo(nodeID)
	if err != nil {
		return false, err
	}
	return api.TolerationsMatchTaints(pod.Spec.Tolerations, node.Status.Taints), nil
}
